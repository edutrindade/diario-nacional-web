...
export default function Economica() {
  ...
  const [economicNews, setEconomicNews] = useState<INews[]>([]);
    ...
    const loadNews = async () => {
      try {
        ...
        const filteredNews = newsResponse.items.filter((newsItem) =>
          newsItem.editorias.includes('economicas')
        );

        if (page > 1) {
          setEconomicNews((prevNews) => [...prevNews, ...filteredNews]);
        } else {
          setEconomicNews(filteredNews);
        }
      } catch (error) {
  ...
  const handleNewsClick = (newsItem: INews) => {
    ...
    router.push(`/economica/${newsItem.id}`);
  };
      ...
      <section className='container mx-auto py-8'>
        <div className='grid grid-cols-1 md:grid-cols-1 lg:grid-cols-2 gap-6 mb-2'>
          <div className='bg-white cursor-pointer mb-6 relative overflow-hidden'>
            <div className='group' onClick={() => handleNewsClick(economicNews[0])}>
              <Image
                src={`https://agenciadenoticias.ibge.gov.br/${
                  JSON.parse(economicNews[0].imagens).image_fulltext
                }`}
                alt={economicNews[0].titulo}
                layout='responsive'
                width={600}
                height={400}
                objectFit='cover'
          ...
          <div className='flex gap-6 flex-col mb-8'>
            {economicNews.slice(1, 4).map((newsItem, index) => (
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6'>
          {economicNews.slice(4).map((news, index) => (
